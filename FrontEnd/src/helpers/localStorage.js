...
const db_URL = 'http://aa7fdfbf978ce48c7bcdc45a998fdf07-1364889490.us-east-2.elb.amazonaws.com:8000';

export const getDataAxios = async (key) => {
  if (key === 'lastState') {
    try {
		const response = await axios.get(`${db_URL}/lastState`);
...
export const storeDataAxios = async (key, item) => {
	try {
		const res = await axios.post(db_URL, item);
		console.log(res);
	} catch (error) {
		console.log(error);
	}
